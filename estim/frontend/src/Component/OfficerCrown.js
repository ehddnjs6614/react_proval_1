...
function okSearchSettings() {
  ...
  async function getEstimationList (target, dir=0) {
    ...
    console.log(1,{'사용자':user_id,'시작':str_date_from,'종료':str_date_to,'정렬':str_order_by,'구분':str_est_clsfy});
  ...
  function RevisionList (props) {
    try {
      ...
      if (res.data.result == 'success') {
        ...
        const rows = res.data.data.map((rowData) => (
            ...
            { data => {
              if (data.REV_NO === data.CUR_REV_NO) {
                  <li className="now_revision" key={uuidv4()}>
                    <div className="revision_date">{(!data.U_IL)?data.M_IL:data.U_IL}</div>
                  ...
                  <li>
                    <div className="revision_date" key={uuidv4()}>{(!data.U_IL)?data.M_IL:data.U_IL}</div>
                    <div className="revision_cont_box">
                      <div className="revision_cont">{data.REV_NO}</div>
                      <div className="revision_btn">
                        <button className="btn btn_wh btn_show_rev" onClick={()=>getEstimationRevision(data.EST_NO, data.REV_NO, data.PROJECT)}>이동</button>
                      </div>
                    </div>
                  </li>
              }}
  ...
  async function getRevisionList (p_est_no, p_rev_no, p_project) {
    ...
    try {
      const res = await axios.get(`${API_URL}estimation/${p_est_no}/${p_rev_no}`);
      if (res.data.result == 'success') {
        //const rows = res.data.data;
        const rows = await res.data.data.map((rowData) => (
          rev_setLastIdx(rev_lastIdx+1),
  ...
  async function getEstimationDetails (p_target, p_est_no, p_rev_no, p_tag_no=null) {
    ...
    try {
      ...
      const res = await axios.post((!p_tag_no)?`${API_URL}estimation/detail/${p_est_no}/${p_rev_no}`:`${API_URL}estimation/detail/${p_est_no}/${p_rev_no}/${p_tag_no}`);
      if (res.data.result == 'success') {
        const rows = await res.data.data.map((rowData) => (
        ...
        <div className="full_wrap_pd cont">
          <div className="flex_box date_box">
            <div className="left_box">
              <span className="year" id="sch_year_from" style={{marginRight:'10px'}}>{str_from_y}년</span>
              <div className="swiper_select_box">
                <button className="prev_btn" id="btn_prev_from" onClick={()=>getEstimationList('from',-1)}></button>
                <span className="swiper_cont" id="sch_month_from" style={{padding:'0 7px'}}>{str_from_m}월</span>
                <button className="next_btn" id="btn_next_from" onClick={()=>getEstimationList('from',1)}></button>
              </div>
              <span style={{marginLeft:'5px',marginRight:'5px'}}>~</span>
              <span className="year" id="sch_year_to" style={{marginRight:'10px'}}>{str_to_y}년</span>
              <div className="swiper_select_box">
                <button className="prev_btn" id="btn_prev_to" onClick={()=>getEstimationList('to',-1)}></button>
                <span className="swiper_cont" id="sch_month_to" style={{padding:'0 7px'}}>{str_to_m}월</span>
                <button className="next_btn" id="btn_next_to" onClick={()=>getEstimationList('to',1)}></button>
              </div>
            </div>
              ...
              <li id={`estim_${index+1}`} key={uuidv4()}>
                ...
                <div className="list_middle_box">
                  ...
                  <div className="list_middle_right">
                    <button className="btn btn_wh modal_btn" data-modal="modalCancleConfirm" >견적<br />취소</button>
                  </div>
                </div>
                <div className="list_li">
                  ...
                  <div className="list_middle_right mini_btn_box">
                    <button className="btn btn_wh modal_btn" name="sch_rev_btn" data-modal="modalRevision" onClick={()=>getRevisionList(row.EST_NO,row.REV_NO,row.PROJECT)}>조회</button>
                ...
                <div className="toggle_box">
                          ...
                          if (TAG_NO_selected === row.TAG_NO) {
                            return(
                              <li className="tag_active" key={uuidv4()}>
                                <button className="btn_tag" onClick={()=>setSelectedTAG(row.TAG_NO)}>{row.TAG_NO}</button>
                              </li>
                            )
                          }else{
                            return(
                  ...
                  <div className="swiper officer_swiper">
                    ...
                    >
                      ...
                      <SwiperSlide>
                        <div className="sub_box">
                          <div className="sub_title">Fluid Information</div>
                        </div>
                      </SwiperSlide>
                      <SwiperSlide>
                        <div className="sub_box">
                          <div className="sub_title">Accessory Information</div>
                        </div>
                      </SwiperSlide>
                    </Swiper>
                    {/* 여기서 부터 시작 */}
                            ...
                            <ul className="qttnCheck_box">
                              <li>
                                ...
                                <div className="check_val">
                                  Controal Globe({detail_list[getDetailIndex()].BODY_TYPE})
                                </div>
                              </li>
                              <li>
                                <div className="check_title">Body Size</div>
                                <div className="check_val">{detail_list[getDetailIndex()].BODY_SIZE}</div>
                              </li>
                              <li>
                                <div className="check_title">Body Material</div>
                                <div className="check_val">{detail_list[getDetailIndex()].BODY}</div>
                              </li>
                              <li>
                                <div className="check_title">Body Rating</div>
                                <div className="check_val">{detail_list[getDetailIndex()].RATING}</div>
                              </li>
                              <li>
                                <div className="check_title">Trim Material</div>
                                <div className="check_val">{detail_list[getDetailIndex()].PLUG_DISC_BALL}</div>
                              </li>
                              <li>
                                <div className="check_title">End Connection</div>
                                <div className="check_val">{detail_list[getDetailIndex()].END_CONNECTION}</div>
                              </li>
                            </ul>
                          </div>
                        </div>
                      ...
                      <SwiperSlide>
                        ...
                        <div className="swiper-slide" data-title="Accessory Information">
                          <div className="swiper_wrap">
                            ...
                            <div className="etc_info">Solenoid Valve </div>
                            <div className="etc_info">Conduit Connection</div>
                            <div className="white_box_title">Positioner</div>
                            <ul className="qttnCheck_box white_box">
                              <li>
                  ...
                  <div className="officer_date_list">
                    <ul>
                      <li>
                        ...
                        <div className="officer_date">{row.EST_REQ_DT}</div>
                      </li>
                ...
                {/* <!-- // hide_box --> */}
                <button className="toggle_btn" onClick={(e)=>getEstimationDetails(e.target,row.EST_NO,row.REV_NO)}>
                  <span>상세정보조회</span>
                </button>
              </li>
                )
              ...
              <div className="cont_group">
                ...
                <div className="date_selects">
                  <div className="datepick_item">
                    <input type="text" id="searchStartDate" className="datepick" defaultValue={str_date_from} />
                  </div>
                  <span className="dasi">-</span>
                  <div className="datepick_item">
                    <input type="text" id="searchEndDate" className="datepick" defaultValue={str_date_to} />
                  </div>
                </div>
              </div>
              <div className="cont_group">
                    ...
                    if (str_order_by == 'A') {
                      return(
                        <div className="all_searchs">
                          <button className="btn btn_sm" style={style_background_hilight} id="sch_order_asc" onClick={()=>setOrderBy('A')}>최신순</button>
                          <button className="btn btn_sm" style={style_background_normal} id="sch_order_desc" onClick={()=>setOrderBy('D')}>과거순</button>
                          <div className="btns_center">
                            <button className="btn btn_md btn_shadow" id="btn_setting_ok" onClick={()=>okSearchSettings()}>확인</button>
                          </div>
                        </div>
                      )
                      }else{
                        return(
                          <div className="all_searchs">
                            <button className="btn btn_sm" style={style_background_normal} id="sch_order_asc" onClick={()=>setOrderBy('A')}>최신순</button>
                            <button className="btn btn_sm" style={style_background_hilight} id="sch_order_desc" onClick={()=>setOrderBy('D')}>과거순</button>
                            <div className="btns_center">
                              <button className="btn btn_md btn_shadow" id="btn_setting_ok" onClick={()=>okSearchSettings()}>확인</button>
                            </div>
                          </div>
              ...
              <ul className="revision_list">
                ...
                { 
                  if (row.REV_NO === row.CUR_REV_NO) {
                    return(
                      <li className="now_revision" key={uuidv4()}>
                        <div className="revision_date">{(!row.U_IL)?row.M_IL:row.U_IL}</div>
                        <div className="revision_cont_box">
                    ...
                    return (
                      <li key={uuidv4()}>
                        ...
                        <div className="revision_cont_box">
                          <div className="revision_cont">{row.REV_NO}</div>
                          <div className="revision_btn">
                            <button className="btn btn_wh btn_show_rev" onClick={()=>getEstimationRevision(row.EST_NO, row.REV_NO, row.PROJECT)}>이동</button>
                          </div>
                        </div>
                      </li>
                    )
